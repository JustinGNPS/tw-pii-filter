...
若直接印該輪遮掉的筆數，數字會隨對話變長一路往上爬 —— 這是 2026-08-22 一次
真實 Codex 工作階段留下的 log：
...
使用者要回答的問題其實是「**又有沒看過的個資離開我的機器了嗎**」，
而不是「歷史裡總共有幾筆」。所以現在印的是：

```
WARNING 已遮蔽：本輪新增 2 筆個資（TW_ID x1、TW_PHONE_M x1）｜快取命中率 78%
```

**沒有新增就完全不印。** 那種情況沒有任何新資訊，而且每個請求本來就有一行
`POST /... -> 200 …` 的完成紀錄可以證明 proxy 有在工作。

累計狀況看 `/healthz`：

```json
{"mapping_entries": 6, "mapping_by_type": {"TW_ID": 3, "TW_PHONE_M": 3}}
```

「新增」的判定是**不重複的真值**：同一個身分證在一次請求裡出現五次、或在十輪
對話裡各出現一次，都只在第一次算新增（對照表對同一個真值永遠回同一個佔位符）。
...
**base URL 設了但連不上**（位址打錯、VPN 沒開、上游掛掉）也是同一套處理，
狀態碼分兩種：

| 情況 | 狀態碼 | `code` |
|---|---|---|
| 連不上（DNS 查不到、連線被拒） | `502` | `upstream_unreachable` |
| 逾時（連得到，只是太慢） | `504` | `upstream_timeout` |

逾時的訊息會附上**目前的秒數設定**，方便判斷是不是自己把 `PROXY_READ_TIMEOUT`
設太短。兩種訊息都會把原始的英文錯誤（例如 `getaddrinfo failed`）附在後面 ——
中文說明給人看，原始訊息拿來查問題。

> 這只涵蓋**開始串流之前**的失敗。若上游是在 SSE 串流中途斷線，回應標頭早就
> 送出去了，狀態碼改不了 —— 那種情況只能讓連線中斷，由 agent 自己判斷。
> 這是 HTTP 的限制，不是實作選擇。
...
**設定沒生效時，沒有任何東西會出聲。** agent 照常運作、照常回答問題，只是內容
直接送去雲端 —— 畫面上不會有一行字說「你的過濾器沒在運作」。

2026-08-22 實測時真的發生了：Codex 因為環境變數沒設成功，讀到另一份設定檔，
`base_url` 直接指著上游，整包繞過 proxy。那次是靠上游回 401 才被發現 ——
**如果上游金鑰剛好是對的，就不會有任何跡象。**
...
**2. 第一個請求抵達時**印一行（只印一次，不會洗版）：

```
第一個請求已抵達 proxy（POST /chat/completions）—— 流量確實有走這裡
```
...
```json
{"traffic": {"requests": 12, "first_at": "2026-08-22T12:41:26",
             "last_at": "2026-08-22T12:41:51", "seconds_since_last": 3.4}}
```

**`requests` 是 0 就代表 agent 從來沒連過來**，去檢查它的 base URL 設定。
健康檢查自己不計入這個數字（否則它永遠不會是 0，也就失去判斷力）；
上游失敗（401、逾時）的請求**仍然計入**，因為那同樣證明 agent 有連過來 ——
正好是最需要診斷時的線索。
...
瀏覽器開 <http://localhost:8010/demo>。

貼一段文字進去，立刻看到左右並排的「你打的」與「雲端 AI 會看到的」、偵測明細
（型別／來源／位置／換成什麼）、組合風險分數與建議、耗時與快取命中率，
以及 proxy 目前的設定與 agent 流量狀態。

**純本機運算，不會送去上游、不消耗任何 API 額度。**

| 設定 | 變數 | 預設 |
|---|---|---|
| 示範頁面 | `PII_ENABLE_DEMO` | 關閉 |

**預設關閉是刻意的**：這個頁面會顯示未遮蔽的原文與對照表（明文個資），而且
提供一個不需認證就能呼叫偵測核心的端點。關閉時 `/demo` 底下一律回 `404`
（不是 `403` —— 沒開就等於不存在）。搭配 uvicorn 預設只綁 `127.0.0.1`，
它不會出現在區域網路上。

示範用的對照表與快取**跟 agent 正在用的那組分開**，在頁面上試打不會讓 agent
的佔位符跳號。但「現況總覽」看的是**真實**的流量與對照表，因為那一區要回答的
是「proxy 現在的狀態」。
